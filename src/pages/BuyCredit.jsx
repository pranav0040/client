      ...
      <div className="flex flex-wrap justify-center gap-6 text-left"> 
        { plans.map((item,index)=>(
            ...
            <p className="mt-6">
              ...
              <button onClick={()=>paymentRazorpay(item.id)} className="w-full bg-gray-800 text-white mt-8 text-sm rounded-md py-2.5 min-2-52">{user?'Purchase':'Get Started'}</button>
            </div>
          ))}
      </div>
    </div>
